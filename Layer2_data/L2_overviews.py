...
def treemap(condition:str):
    ...
    if condition not in l2_condition:
        ...
        return data[['chain','unique_users','size']].rename(columns={'chain':'label','unique_users':'value'}).to_dict(orient="records")
    elif condition =='amount':
        size =[500,250,150,100,60,40,32,30,26,10]
        data['size'] = [i for i in size[:len(data)]]
        return data[['chain','eth_amount','size']].rename(columns={'chain':'label','eth_amount':'value'}).to_dict(orient="records")
    elif condition =='fee':
        size =[500,250,150,100,60,40,32,30,26,10]
        data['size'] = [i for i in size[:len(data)]]
        return data[['chain','fee_tx','size']].rename(columns={'chain':'label','fee_tx':'value'}).to_dict(orient="records")
    elif condition =='tx':
        size =[500,250,150,100,60,40,32,30,26,10]
        data['size'] = [i for i in size[:len(data)]]
...
def create_statics_L2(condition:str):
    ...
    if condition not in l2_condition:
        ...
        return data[['chain','unique_users']].rename(columns={'chain':'label','unique_users':'value'}).to_dict(orient='records')
    elif condition =="amount":
        return data[['chain','eth_amount']].rename(columns={'chain':'label','eth_amount':'value'}).to_dict(orient='records')
    elif condition =="fee":
        return data[['chain','fee_tx']].rename(columns={'chain':'label','fee_tx':'value'}).to_dict(orient='records')
    elif condition =="tx":
        return data[['chain','tx']].rename(columns={'chain':'label','tx':'value'}).to_dict(orient='records')
    ...
    def Daily(l2:str,start:str,end:str,col_condition:str):
       ...
       'Base', 'Mantle', 'Linea', 'Manta', 'Scroll']
        ...
        if col_condition == 'unique_users':
            ...
            return data[cols].rename(columns={'dt':'timestamp','chain':'label','unique_users':'value'}).to_dict(orient="records")
        elif col_condition=='eth_amount':
            cols =['dt','chain','eth_amount']
            return data[cols].rename(columns={'dt':'timestamp','chain':'label','eth_amount':'value'}).to_dict(orient="records")
        elif col_condition=='tx':
            cols =['dt','chain','tx']
            return data[cols].rename(columns={'dt':'timestamp','chain':'label','tx':'value'}).to_dict(orient="records")
